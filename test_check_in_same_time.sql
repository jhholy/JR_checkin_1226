-- 测试脚本：验证会员可以在同一天同一时间段同时签到团课和私教课
...
BEGIN
  ...
  BEGIN
    ...
    RAISE NOTICE '测试成功：会员可以在同一天同一时间段同时签到团课和私教课';
  EXCEPTION
    WHEN OTHERS THEN
      RAISE NOTICE '创建私教课签到失败: %', SQLERRM;
      RAISE NOTICE '测试失败：会员无法在同一天同一时间段同时签到团课和私教课';
  END;
